...
Upon testing, I observed that my system outperforms the regular transformer on 5/
...
4.3 Future Work
4 References
Figure 1. A four-node neural network. List of Illustrations
Figure 2. A recurrent neural network
Figure 3. Example of BLEU scoring.
Figure 4. The Encoder and Decoder layers of a Transformer
Figure 5. The positional embedding equation (for even indices)
Figure 6. The positional embedding equation (for odd indices)..
Figure 7. Using column vectors of constants (e.g. 0,1,2,3) as position encodings
...
To have a Transformer that can produce multiple candidate outputs instead of just one.
To observe if I can use a function to select from multiple candidate outputs to construct a
...
strings, hence they are similar challenges^1. Therefore, our research will develop a formalism that
...
consideration here only work with strings, and the task we face with such models is to "mimic" structure with strings alone.
...
Neural machine translation started with the use of recurrent neural networks in 201 4
...
them are weights^2. The Figure is from Matt Mazur, (https://mattmazur.com/2015/03/17/a-step-
...
would equal:
𝑤 1 ∙𝑖 1 +𝑤 2 ∙𝑖 2 =. 15 ∙. 05 +. 25 ∙. 10

(^2) The example outlined here is a fully^ - connected network , which means that each node of a layer is
connected to every node of the following layer. A network need not be fullyof a previous layer have to be connected to the nodes of the following layer via weights), but since Transformers use -connected (meaning not all of the nodes
...
(https://towardsdatascience.com/recurrent-neural-networks-rnn-explained-the-eli5-way-
3956887e8b75)
...
for linguistic analysis. This is commonly assumed in the semantic parsing literature, so I
...
in the lambda-calculus: 𝜆 x 𝜆 y. borders(y,x); this takes an argument for x, here t, yielding a new
function 𝜆 y. borders(y, t) ; this then applies to a , yielding borders(a,t).
...
The part of the question before the word border are identical to the above example: 𝜆 x.
...
know that the lambda calculus formula associated with a sentence properly captures its truth-
conditions? Once again, answering this question in an automated manner is not easy since there
are multiple ways of representing semantically equivalent formulas. For example: "book(a)" and
"lambda x. book(x)" are semantically equivalent representations of a is a book.
...
algorithmic level. For example, active and passive variants of a sentence are arguably truth-
conditionally equivalent but they may have differences that are relevant in some sense to
...
𝑠𝑜𝑓𝑡𝑚𝑎𝑥(𝑞 ∙𝑘𝑇)∙𝑣
where ∙ represents matrix multiplication, 𝑘𝑇represents the key vector transposed and softmax is a
...
[
(𝑗𝑒|𝑗𝑒) (𝑗𝑒|𝑠𝑢𝑖𝑠) (𝑗𝑒|é𝑡𝑢𝑑𝑖𝑎𝑛𝑡)
(𝑠𝑢𝑖𝑠|𝑗𝑒) (𝑠𝑢𝑖𝑠|𝑠𝑢𝑖𝑠) (𝑠𝑢𝑖𝑠|é𝑡𝑢𝑑𝑖𝑎𝑛𝑡)
(é𝑡𝑢𝑑𝑖𝑎𝑛𝑡|𝑗𝑒) (é𝑡𝑢𝑑𝑖𝑎𝑛𝑡|𝑠𝑢𝑖𝑠) (é𝑡𝑢𝑑𝑖𝑎𝑛𝑡|é𝑡𝑢𝑑𝑖𝑎𝑛𝑡)
]∙[ 𝑠𝑢𝑖𝑠𝑗𝑒
é𝑡𝑢𝑑𝑖𝑎𝑛𝑡
]
where (𝑦|𝑥) represents the probability of y given x. The matrix containing the (𝑦|𝑥) terms is
...
(𝑎𝑚|𝐼) (𝑎𝑚|𝑎𝑚) 0 0
(𝑎|𝐼) (𝑎|𝑎𝑚) (𝑎|𝑎) 0
(𝑠𝑡𝑢𝑑𝑒𝑛𝑡|𝐼) (𝑠𝑡𝑢𝑑𝑒𝑛𝑡|𝑎𝑚) (𝑠𝑡𝑢𝑑𝑒𝑛𝑡|𝑎) (𝑠𝑡𝑢𝑑𝑒𝑛𝑡|𝑠𝑡𝑢𝑑𝑒𝑛𝑡)
]∙[
𝐼
𝑎𝑚
𝑎
...
[^
(𝐼|𝑗𝑒) (𝐼|𝑠𝑢𝑖𝑠) (𝐼|é𝑡𝑢𝑑𝑖𝑎𝑛𝑡)
(𝑎𝑚|𝑗𝑒) (𝑎𝑚|𝑠𝑢𝑖𝑠) (𝑎𝑚|é𝑡𝑢𝑑𝑖𝑎𝑛𝑡)
(𝑎|𝑗𝑒) (𝑎|𝑠𝑢𝑖𝑠) (𝑎|é𝑡𝑢𝑑𝑖𝑎𝑛𝑡)
(𝑠𝑡𝑢𝑑𝑒𝑛𝑡|𝑗𝑒) (𝑠𝑡𝑢𝑑𝑒𝑛𝑡|𝑠𝑢𝑖𝑠) (𝑠𝑡𝑢𝑑𝑒𝑛𝑡|é𝑡𝑢𝑑𝑖𝑎𝑛𝑡)]^
(^) ∙[ 𝑠𝑢𝑖𝑠𝑗𝑒
é𝑡𝑢𝑑𝑖𝑎𝑛𝑡
...
are used to calculated self-attention mappings exactly as outlined above. Each of these self-
attention mappings are called attention heads.
For example, for word vectors of dimension 512 if we had 8 attention heads, this would mean
that our k,t and q vectors would be of dimension 512/8=64. That would mean, for each self-
attention layer, we would have 8 outputs of dimension 64 instead of 1 of dimension 512.
After calculating 𝑠𝑜𝑓𝑡𝑚𝑎𝑥(𝑞 ∙𝑘𝑇)∙𝑣 for all the attention heads, in order to get back to
...
Seeing if the exact match accuracy and the BLEU scores of benchmark neural machine
translation and semantic parsing datasets can be increased by using this many-weight
approach.
Seeing if semantically equivalent variants of correct translations and parses can be
...
book(x) are semantically equivalent representations of a is a book. In fact, in tests I ran on
...
where, as we saw on page 28: depth = 𝑑𝑖𝑚𝑒𝑛𝑠𝑖𝑜𝑛# 𝑎𝑡𝑡𝑒𝑛𝑡𝑖𝑜𝑛^ 𝑜𝑓^ 𝑤𝑜𝑟𝑑 ℎ𝑒𝑎𝑑𝑠^ 𝑣𝑒𝑐𝑡𝑜𝑟𝑠.
...
the 𝑘𝑇vectors yield the attention weight matrix with the dimensions we outlined.
Notice that the order of the last two dimensions ( sequence length and depth ) of both of
the vectors are reversed. The last two dimension of 𝑞 is like a matrix of dimensions:
( sequence length of q, depth), whereas for 𝑘𝑇, this is (depth, sequence length of k). Notice that
very much like the matrix multiplication of matrices: (m,n) ∙ (n,k), the dimensions of the column
of q and the dimensions of the row of 𝑘𝑇agree. Therefore, their matrix multiplication will give:
(batch size, # attention heads, sequence length of q, depth)
∙
(batch size, # attention heads, depth, sequence length of k)
=
...
(𝑠𝑡𝑢𝑑𝑒𝑛𝑡|𝑗𝑒) (𝑠𝑡𝑢𝑑𝑒𝑛𝑡|𝑠𝑢𝑖𝑠) (𝑠𝑡𝑢𝑑𝑒𝑛𝑡|é𝑡𝑢𝑑𝑖𝑎𝑛𝑡)]^
𝑑𝑒𝑝𝑡ℎ
where the subscripts of the attention weights denote their index. In our formulation we have depth-
many attention weights per attention weight instead of one.
...
[^
(^) (𝑎𝑚(𝐼|𝑗𝑒|𝑗𝑒)^7 ) 8 (𝑎𝑚(𝐼|𝑠𝑢𝑖𝑠|𝑠𝑢𝑖𝑠)^1 ) 2 (𝑎𝑚(𝐼|é|𝑡𝑢𝑑𝑖𝑎𝑛𝑡é𝑡𝑢𝑑𝑖𝑎𝑛𝑡)^3 ) 6
(𝑎|𝑗𝑒) 1 (𝑎|𝑠𝑢𝑖𝑠) 4 (𝑎|é𝑡𝑢𝑑𝑖𝑎𝑛𝑡) 4
(𝑠𝑡𝑢𝑑𝑒𝑛𝑡|𝑗𝑒) 9 (𝑠𝑡𝑢𝑑𝑒𝑛𝑡|𝑠𝑢𝑖𝑠) 11 (𝑠𝑡𝑢𝑑𝑒𝑛𝑡|é𝑡𝑢𝑑𝑖𝑎𝑛𝑡) 1 ]^
where the subscripted indices of the entries represented which attention weight the number was
selected from. For example, (𝐼|𝑗𝑒) 7 would mean that for all attention weights that our algorithm
produced, it was the 7th attention weight that gave the highest score for the calculation (𝐼|𝑗𝑒),
...
(𝐼|𝑎𝑚) (𝑎𝑚|𝑎𝑚) 0 0
(𝐼|𝑎) (𝑎𝑚|𝑎) (𝑎|𝑎) 0
(𝐼|𝑠𝑡𝑢𝑑𝑒𝑛𝑡) (𝑎𝑚|𝑠𝑡𝑢𝑑𝑒𝑛𝑡) (𝑎|𝑠𝑡𝑢𝑑𝑒𝑛𝑡) (𝑠𝑡𝑢𝑑𝑒𝑛𝑡|𝑠𝑡𝑢𝑑𝑒𝑛𝑡)
]
...
reduced the dimension of my word vectors from 512 to 128, which gave us a depth of^1288 = 16
...
Our
Transformer
Tatoeba EnglishGerman - 20.44 % 21.6%
IWSLT'15 English-
Vietnamese
...
algorithm uses our proposed MAWT self-attention mechanism in which there are multiple self-
attention matrices instead of one. As we explained in detail on Section 2, self-attention matrices
...
Table Datasets 5. Average BLEU scores for Regular versus Our Transformer for Semantic Parsing

Dataset Regular Transformer Our Transformer
Geo880 90.9%
90.4%
ATIS 89.9%
90.2%
...
28.38%
28.2%
IWSLT'15 VietnameseEnglish - 15.59% 15.59%
Tatoeba English-
Greek
14.13% 14.7%
...
Sie wollen, dass ich
gehe, oder?
You're fashionable. Sie sind modebewusst. Sie sind modebewusst. Du bist modebewusst.
...
compared the exact match accuracy and BLEU score results of our and the regular Transformer^3.

Table 8. Exact Match Accuracies for Translations of 50 0 Most Frequent Words Used in Neural
...
Vietnamese
(^) 30.8% 19 .2%
Tatoeba Greek English- 17.6% 16.4%
(^3) We didn’t perform this task for semantic parsing, b^ ecause there is no engine like Google Translate we
...
Table 9. Average BLEU Scores for Translations of 50 0 Most Frequent Words Used in Neural
Machine Translation Datasets.
Dataset Regular
Transformer
Our
...
Table 11. Exact Match Accuracies for Translations of 500 Most Frequent Words Used in Neural
Machine Translation Datasets.
Dataset Regular Transformer Our Transformer Hybrid Transformer
Tatoeba EnglishGreek - 17.4% 16.4% 11.21%
...
Hybrid
Transformer
Tatoeba EnglishGreek - 38.82% 33.5% 22.75%

As can be seen from Table 1 1 and 1 2 , the hybrid transformer underperformed on word
...
Therefore, merely combining our algorithm with the regular transformer by creating a two-
layered hybrid model as we did does not suffice to create an algorithm that improves sentential
...
Dataset Vocabulary Average sentence
length of the
train set
...
Observing my findings on Table 1 3 , firstly, it is evident that the difference in the three
...
equivalence. For example, the sentences only Sam called and Sam called are not truth-
conditionally equivalent, but in response to the question Which of Sam, Kim, and Pat called they
...
arXiv:1606.03622.
Kearns, K. (200 0 ). Semantics. Macmillan.
